...
namefile='prec_ITPCAS-CMFD_V0106_B-01_01dy_010deg_197901-197912.nc';
...
kk=1;
Prec_Yangkun_all=cell(length( start_day(1):end_day(1)),1);
...
for year = start_day(1):end_day(1)
    ...
    for jj=1:yeardays(year)
        ...
        parfor i=1:length(ID_lon_all)
            PCP_Temp=Prec_Yangkun(ID_lon_all(i),ID_lat_all(i)); % read data for current station
 ...
 for mm=1:length(lat_IRR)
     ...
     current_data=sum(data_temp(current_start:current_end)); % mm/growth period
     data_all(mm)=current_data;  
 end
  
  %%  
    Prec_Yangkun_all{kk}=data_all;
     kk=kk+1;
end

save 'Prec_Yangkun_all.mat', 'Prec_Yangkun_all';
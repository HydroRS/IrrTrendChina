...
namefile='srad_ITPCAS-CMFD_V0106_B-01_01dy_010deg_197901-197912.nc';
...
kk=1;
Srad_Yangkun_all=cell(length( start_day(1):end_day(1)),1);
...
for year = start_day(1):end_day(1)
    ...
    for jj=1:yeardays(year)
        ...
        Srad_Yangkun=ncread([Location_Yangkun,'srad_ITPCAS-CMFD_V0106_B-01_01dy_010deg_',num2str(year),'01-',num2str(year),'12.nc'],'srad',[1 1,jj],[700,400,1]); % unit:w/m2
        
        
        PCP_day=[];
        parfor i=1:length(ID_lon_all)
            PCP_Temp=Srad_Yangkun(ID_lon_all(i),ID_lat_all(i)); % read data for current station
 ...
 for mm=1:length(lat_IRR)
  ...
  %%  
    Srad_Yangkun_all{kk}=data_all;
     kk=kk+1;
end

save 'Srad_Yangkun_all.mat', 'Srad_Yangkun_all';
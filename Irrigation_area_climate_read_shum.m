...
namefile='Shum_ITPCAS-CMFD_V0106_B-01_01dy_010deg_197901-197912.nc';
...
kk=1;
Shum_Yangkun_all=cell(length( start_day(1):end_day(1)),1);
...
for year = start_day(1):end_day(1)
    ...
    for jj=1:yeardays(year)
        ...
        Shum_Yangkun=ncread([Location_Yangkun,'Shum_ITPCAS-CMFD_V0106_B-01_01dy_010deg_',num2str(year),'01-',num2str(year),'12.nc'],'shum',[1 1,jj],[700,400,1])*1000; % unit:Kg/kg-->g/kg
        
        
        PCP_day=[];
        parfor i=1:length(ID_lon_all)
            PCP_Temp=Shum_Yangkun(ID_lon_all(i),ID_lat_all(i)); % read data for current station
 ...
 for mm=1:length(lat_IRR)
     ...
     current_data=mean(data_temp(current_start:current_end)); % w/m2 during the growth period
     data_all(mm)=current_data;  
 end
  
  %%  
    Shum_Yangkun_all{kk}=data_all;
     kk=kk+1;
end

save 'Shum_Yangkun_all.mat', 'Shum_Yangkun_all';